...
const VideoPlayer = ({ socket, currentVideo, userRole, videoState, playlist, onVideoSelect }) => {
	...
	// New: mark when a local playlist selection happened so we can autoplay only for selections
	const lastLocalSelect = useRef(false);
	...
	useEffect(() => {
		...
		// modified: notify parent about currentVideo so guests update their currentVideo prop/UI
		const handleVideoChanged = (data) => {
			...
			// Always let parent know about the new currentVideo so the guest UI/playlist highlights update
			try {
				onVideoSelect?.(data.currentVideo);
			} catch (err) {
				console.error("SOCKET: onVideoSelect callback error:", err);
			}
	...
	}, [socket, userRole, isReady, onVideoSelect]);

	// react when currentVideo prop changes (user clicked a playlist item or parent changed it)
	useEffect(() => {
		...
		if (lastChangeBySocket.current) {
			...
			// ensure videoState is applied for socket-origin changes
			return;
		}

		// If this was a deliberate local selection (playlist click/window event), follow the autoplay/emit flow
		if (lastLocalSelect.current) {
			// Reset the local-selection marker now that we're handling it
			lastLocalSelect.current = false;

			// Local selection behavior (likely cohost)
			...
			// For guests or when no socket, if it was a local selection, just attempt to load (do not autoplay)
			try {
				playerRef.current.loadVideoById(currentVideo.videoId);
			} catch (err) {
		...
		// Not a local selection and not a socket-origin change — likely a programmatic change (e.g., set by host UI).
		// Load the video locally but do NOT autoplay. Additionally, if we're a cohost, broadcast the change so all clients sync.
		log("PLAYER", "Programmatic currentVideo change — loading without autoplay and synchronizing other clients");

		try {
			playerRef.current.loadVideoById(currentVideo.videoId);
		} catch (err) {
			console.error("PLAYER: non-select loadVideoById error:", err);
		}
		...
		if (userRole === "cohost" && socket) {
			...
			socket.emit("video-changed", { currentVideo, videoState: videoStateToSend });
			log("PLAYER", "Broadcasted video-changed for programmatic change", { currentVideo, videoStateToSend });
		}
	...
	useEffect(() => {
		const handler = (e) => {
			...
			// Safety: ignore if guest (Playlist should already avoid dispatching for guests, but double-check)
			if (userRole === "guest") {
				log("UI", "Ignoring playlist selection event because user is a guest");
				return;
			}
			...
			// Mark as a local selection so the currentVideo effect will know to autoplay
			lastLocalSelect.current = true;
			lastChangeBySocket.current = false;
	...
	return (
		<div className="relative">
			...
			<div className="absolute top-2 right-2 z-20">
				...
				>
					{userRole === "cohost" ? (
						<span className="inline-flex items-center"><FaCrown className="mr-1" />Cohost</span>
					) : (
						<span className="inline-flex items-center"><FaUser className="mr-1" />Guest</span>
					)}
				</div>
			</div>
